...
class ConvergentSeries(Scene):
    def construct(self):
        ...
        nfgsum_tex = MathTex(r'{{r}}', '\sum_{n=0}^{N}', r'{{a}}{{r}}^{{n}}').set_color_by_tex('a', RED).set_color_by_tex('r', color=GREEN).set_color_by_tex('n', YELLOW)
        ...
        self.play(TransformMatchingTex(fgsum_tex, nfgsum_tex), TransformMatchingTex(expanded_sum, nexpanded_sum))
        self.wait(1)
